// Set the date for New Year's Day 2025
const newYearsDate = new Date('January 1, 2025 00:00:00'); 

// Update the countdown every second
const countdownInterval = setInterval(() => {
    const now = new Date();
    const timeRemaining = newYearsDate - now;

    // If New Year's has passed, update to the next year
    if (timeRemaining < 0) {
        newYearsDate.setFullYear(newYearsDate.getFullYear() + 1);
    }
...
function makeChar(c){
    ...
    let size = tmp.width = tmp.height = w<500?300:600;
